"""Train and evaluate a TF-IDF + ExtraTreesClassifier sentiment classifier."""
...
DEFAULT_OUTPUT_DIR = OUTPUTS_DIR / "baselines" / "extra_trees"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train and evaluate ExtraTreesClassifier on Kaggle IMDb reviews.",
    )
    parser.add_argument(
        "--csv-path",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=42,
        help="Random seed for the stratified train/test split and trees.",
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    parser.add_argument(
        "--bootstrap",
        action="store_true",
        help="Use bootstrap samples instead of the full training set for each tree.",
    )
    parser.add_argument(
        "--verbose",
        type=int,
        default=0,
        help="Verbosity passed to ExtraTreesClassifier.",
    )
...
def main() -> None:
    ...
    print(f"Saved ExtraTreesClassifier artifacts to {args.output_dir}")
