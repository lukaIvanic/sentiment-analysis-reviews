"""Train and evaluate a TF-IDF + XGBoostClassifier sentiment classifier."""
...
DEFAULT_OUTPUT_DIR = OUTPUTS_DIR / "baselines" / "xgboost_classifier"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train and evaluate XGBoostClassifier on Kaggle IMDb reviews.",
    )
    parser.add_argument(
        "--csv-path",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=42,
        help="Random seed for the stratified train/test split and XGBoost.",
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Maximum depth of each boosted tree.",
    )
    parser.add_argument(
        "--subsample",
        type=float,
        default=0.8,
        help="Fraction of training rows sampled for each tree.",
    )
    parser.add_argument(
        "--colsample-bytree",
        type=float,
...
def main() -> None:
    ...
    print(f"Saved XGBoostClassifier artifacts to {args.output_dir}")
