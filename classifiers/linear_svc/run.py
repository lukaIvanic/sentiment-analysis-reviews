"""Train and evaluate a TF-IDF + LinearSVC sentiment classifier."""
...
DEFAULT_OUTPUT_DIR = OUTPUTS_DIR / "baselines" / "linear_svc"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train and evaluate LinearSVC on Kaggle IMDb reviews.",
    )
    parser.add_argument(
        "--csv-path",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Regularization parameter. Larger values regularize less strongly.",
    )
    parser.add_argument(
        "--max-iter",
        type=int,
...
def main() -> None:
    ...
    print(f"Saved LinearSVC artifacts to {args.output_dir}")
    ...
    print("Log-loss: unavailable for uncalibrated LinearSVC")
