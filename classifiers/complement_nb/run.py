"""Train and evaluate a TF-IDF + ComplementNB sentiment classifier."""
...
DEFAULT_OUTPUT_DIR = OUTPUTS_DIR / "baselines" / "complement_nb"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train and evaluate ComplementNB on Kaggle IMDb reviews.",
    )
    parser.add_argument(
        "--csv-path",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Smoothing value for ComplementNB.",
    )
...
def build_pipeline(args: argparse.Namespace) -> Pipeline:
    ...
    return Pipeline(
        steps=[
            ...
            ("classifier", ComplementNB(alpha=args.alpha)),
        ]
    )
...
def main() -> None:
    ...
    print(f"Saved ComplementNB artifacts to {args.output_dir}")
